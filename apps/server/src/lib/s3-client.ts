...
/**
 * Store metadata as JSON in R2 storage
 * @param metadata The metadata object to store
 * @param prefix Optional path prefix
 * @returns The public URL to the stored metadata
 */
export async function storeMetadata(
  ...
  try {
    ...
    const symbol = metadata.symbol?.toLowerCase() || "token";
    const filename = `${prefix}/${symbol}-${timestamp}-${randomId}.json`;
