...
// ==================== VERTIGO TRANSACTION PROCESSING ====================

/**
 * Process a Vertigo transaction with improved error handling
 */
export const processVertigoTransaction = (
  txData: TransactionData,
  isProgramMuted?: (programId: string) => boolean
) => {
  // Check if this is a Vertigo transaction
  if (!isVertigoTransaction(txData)) {
    return null;
  }

  // Skip transactions from muted programs completely
  if (isProgramMuted && isProgramMuted(VERTIGO_PROGRAM_ID)) {
    return null;
  }

  // Skip failed transactions
  if (txData.transaction?.meta?.err) {
    logWarning(`Skipping failed Vertigo transaction: ${txData.signature}`);
    return null;
  }
  ...
  // Log transaction signature in a compact format
  log(`Vertigo Transaction: ${shortSig}`, "info");

  // Extract token transfers for analysis - Vertigo transactions often involve token transfers
  const tokenTransfers = extractTokenTransfers(txData);
  const solTransfers = extractSolTransfers(txData);
  const tokenBalanceChanges = extractTokenBalanceChanges(txData);

  // Build a more structured representation of the Vertigo transaction
  const vertigoEvent = {
    type: "VertigoTransaction",
    signature: txData.signature,
    slot: txData.slot,
    timestamp: new Date().toISOString(),
    tokenTransfers,
    solTransfers,
    tokenBalanceChanges,
    rawLogs: txData.transaction?.meta?.logMessages || [],
  };

  // Log a clean summary of the Vertigo transaction
  logEvent(`🌀 \x1b[36mVERTIGO\x1b[0m transaction detected: ${shortSig}`);

  if (tokenTransfers && tokenTransfers.length > 0) {
    logEvent(`🔄 Token transfers: ${tokenTransfers.length}`);
  }

  if (solTransfers && solTransfers.length > 0) {
    logEvent(`💰 SOL transfers: ${solTransfers.length}`);
  }
