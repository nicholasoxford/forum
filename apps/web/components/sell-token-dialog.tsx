...
export function SellTokenDialog({
  ...
  const handleSellToken = async () => {
    const success = await sellToken(tokenMint, amount);
    if (success) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            Sell {tokenName} ({tokenSymbol})
          </DialogTitle>
          <DialogDescription>
            Sell your tokens back to the pool and get SOL
          </DialogDescription>
        </DialogHeader>
        ...
        {!connected ? (
          <div className="flex flex-col items-center py-4 space-y-4">
            <p className="text-center text-sm text-zinc-500">
              Connect your wallet to sell tokens
            </p>
            <WalletButton />
          </div>
        ) : (
